...
type WKHTMLImageOptions struct {
...
func GenerateImage(options *WKHTMLImageOptions) ([]byte, error) {
	arr, err := buildParams(options)
	if err != nil {
		return []byte{}, err
	}
	fmt.Printf("Arguements: %v\n", arr)
...
func buildParams(options *WKHTMLImageOptions) ([]string, error) {
	a := []string{}
	...
	a = append(a, "10000")
	a = append(a, "--log-level")
	a = append(a, "info")
	a = append(a, "--no-stop-slow-scripts")
